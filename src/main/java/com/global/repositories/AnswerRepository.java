...
public interface AnswerRepository extends CrudRepository<AnswerIntity, Integer> {
	...
	@Query("select new com.global.classes.StudentResponse(st.name,q.question,ans.answer,ans.score) from AnswerIntity ans join ans.student st join ans.question q where st.teacherName=:teacherName order by st.name")
	...
	@Query("select new com.global.classes.StudentResponseTopFail(st.name,sum(ans.score) as degree) from AnswerIntity ans join ans.student st  where st.teacherName=:teacherName group by st.name  order by sum(ans.score) desc ")
	public List<StudentResponseTopFail> findAllStudents(String teacherName);
	
	
	@Query("select new com.global.classes.StudentResponseTopFail(st.name,sum(ans.score) as degree) from AnswerIntity ans join ans.student st where st.teacherName=:teacherName group by st.name having sum(ans.score) >= :successDegree order by sum(ans.score) desc ")
